  ...
  const createUsers = async () => {
    try {
      ...
      await new Promise((resolve, reject) => {
        db.run(
          `INSERT OR IGNORE INTO users (username, email, password, role) 
                        VALUES ('admin', 'admin@demo.com', ?, 'admin')`,
          [adminHash],
          (err) => {
            if (err) reject(err);
            else resolve();
      ...
      await new Promise((resolve, reject) => {
        db.run(
          `INSERT OR IGNORE INTO users (username, email, password, role) 
                        VALUES ('testuser', 'test@demo.com', ?, 'user')`,
          [userHash],
          (err) => {
            if (err) reject(err);
            else resolve();
      ...
      await new Promise((resolve, reject) => {
        db.run(
          `INSERT OR IGNORE INTO users (username, email, password, role, profile_data) 
                        VALUES ('john_doe', 'john@demo.com', ?, 'user', ?)`,
          [johnHash, '{"name": "John Doe", "bio": "Regular user"}'],
          (err) => {
            if (err) reject(err);
            else resolve();
          }
